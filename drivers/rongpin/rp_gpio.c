				...
				if (flash_flag++ > 10)
					{
						...
						if (flash_counter < 50)
							{
								gpio_direction_output(gpio_data->rp_gpio_num[i].gpio_num,gpio_data->rp_gpio_num[i].action);
								gpio_data->rp_gpio_num[i].action = !gpio_data->rp_gpio_num[i].action;
								flash_flag = 0;
							}
					}
				break;
...
//	enum of_gpio_flags  gpio_flags;
	...
	{
		/* parse dts */
		gpio_data->rp_gpio_num[gpio_cnt].gpio_num = of_get_named_gpio_flags(child_np, "gpio_num", 0, (enum of_gpio_flags *)(&gpio_flags.gpio_cfg));
		if (!gpio_is_valid(gpio_data->rp_gpio_num[gpio_cnt].gpio_num)){
			return -1;
		}		

		gpio_data->rp_gpio_num[gpio_cnt].gpio_name = (char*)child_np -> name;
		gpio_data->rp_gpio_num[gpio_cnt].action = gpio_flags.gpio_cfg.data;
		...
		switch(gpio_data->rp_gpio_num[gpio_cnt].gpio_function) {
			...
			case GPIO_FUNCTION_INPUT :		/* init input gpio */
				...
				if (ret < 0)
				{
					...
					event_flag = gpio_data->rp_gpio_num[gpio_cnt].action;
...
//					printk("success request gpio%d out\n",gpio_data->rp_gpio_num[gpio_cnt].gpio_num);
					...
					printk("success request gpio%d out gpio_data->rp_gpio_num[gpio_cnt].action %d\n",gpio_data->rp_gpio_num[gpio_cnt].gpio_num,gpio_data->rp_gpio_num[gpio_cnt].action);
				}
				break;
	...
	if (gpio_in_cnt > 0)
	{
		...
		gpio_data->mytimer.expires = jiffies + msecs_to_jiffies(1000);
