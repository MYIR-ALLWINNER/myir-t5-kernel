...
atomic_t usb_vbus_cnt = ATOMIC_INIT(0);
atomic_t usb_resume_vbus_cnt = ATOMIC_INIT(0);
...
void sunxi_suspend_set_vbus(struct sunxi_hci_hcd *sunxi_hci)
{
	mutex_lock(&usb_vbus_lock);

	if (sunxi_hci->usbc_no == HCI0_USBC_NO) {
		if (atomic_read(&usb1_set_vbus_cnt) == 1) {
			atomic_add(1, &usb_vbus_cnt);
			__sunxi_set_vbus(sunxi_hci, 0);  /* power off */
		}
		atomic_sub(1, &usb1_set_vbus_cnt);
	} else if (sunxi_hci->usbc_no == HCI1_USBC_NO) {
		if (atomic_read(&usb2_set_vbus_cnt) == 1) {
			atomic_add(1, &usb_vbus_cnt);
			__sunxi_set_vbus(sunxi_hci, 0);  /* power off */
		}
		atomic_sub(1, &usb2_set_vbus_cnt);
	} else if (sunxi_hci->usbc_no == HCI2_USBC_NO) {
		if (atomic_read(&usb3_set_vbus_cnt) == 1) {
			atomic_add(1, &usb_vbus_cnt);
			__sunxi_set_vbus(sunxi_hci, 0);  /* power off */
		}
		atomic_sub(1, &usb3_set_vbus_cnt);
	} else if (sunxi_hci->usbc_no == HCI3_USBC_NO) {
		if (atomic_read(&usb4_set_vbus_cnt) == 1) {
			atomic_add(1, &usb_vbus_cnt);
			__sunxi_set_vbus(sunxi_hci, 0);  /* power off */
		}
...
void sunxi_resume_set_vbus(struct sunxi_hci_hcd *sunxi_hci)
{
	mutex_lock(&usb_vbus_lock);

	if (sunxi_hci->usbc_no == HCI0_USBC_NO) {
		if (atomic_read(&usb1_set_vbus_cnt) == 1) {
			atomic_sub(1, &usb_vbus_cnt);
			atomic_add(1, &usb_resume_vbus_cnt);
		}
	...
	if (atomic_read(&usb_vbus_cnt) == 0) {
		while (atomic_read(&usb_resume_vbus_cnt)) {
			__sunxi_set_vbus(sunxi_hci, 1);  /* power on */
			atomic_sub(1, &usb_resume_vbus_cnt);
		}
	}
