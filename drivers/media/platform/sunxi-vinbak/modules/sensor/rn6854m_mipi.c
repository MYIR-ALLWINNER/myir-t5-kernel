...
static void rn6854m_set_input_size(struct sensor_info *info,
		struct v4l2_subdev_format *fmt, int ch_id)
{
		...
		case CVBS_FRAME_PAL:
			...
			fmt->format.height = 604;
			break;
		case CVBS_FRAME_NTSC:
			fmt->format.width = 720;
			fmt->format.height = 510;
			break;
		default:
			break;
		}
...
int rn6854m_sensor_set_fmt(struct v4l2_subdev *sd,
			...
			struct v4l2_subdev_format *fmt)
{
	...
	if (sd->entity.stream_count == 0) {
		rn6854m_set_input_size(info, fmt, fmt->reserved[0]);
...
static int sensor_ch_reg_init(struct v4l2_subdev *sd,
		struct tvin_init_info *tvin_info)
{
	int ch_id;

	ch_id = tvin_info->ch_id;
	sensor_print("sensor_ch_reg_init tvin_info->ch_id = %d\n", ch_id);
	sensor_initial(sd);
	sensor_write_array(sd, sensor_turn_on_video_decoder,
				 ARRAY_SIZE(sensor_turn_on_video_decoder));
	if (tvin_info->input_fmt[ch_id] == CVBS_FRAME_PAL) {
		sensor_write(sd, 0xFF, ch_id);
		sensor_write(sd, 0x00, 0x00);
		sensor_write(sd, 0x06, 0x08);
		sensor_write(sd, 0x07, 0x62);
		sensor_write(sd, 0x2A, 0x81);
		sensor_write(sd, 0x3A, 0x20);
		sensor_write(sd, 0x3F, ch_id + 0x10);
		sensor_write_array(sd, sensor_cvbs_P_frame_mode_regs,
					 ARRAY_SIZE(sensor_cvbs_P_frame_mode_regs));
		sensor_write_array(sd, sensor_mipi_link_1,
					 ARRAY_SIZE(sensor_mipi_link_1));
		sensor_write(sd, 2 * ch_id + 0x78, 0x68);
		sensor_write(sd, 2 * ch_id + 0x79, 0x01);
		sensor_write_array(sd, sensor_mipi_link_2,
					 ARRAY_SIZE(sensor_mipi_link_2));
		sensor_print("tvin_info->input_fmt[ch_id] == CVBS_FRAME_PAL\n");

	} else if (tvin_info->input_fmt[ch_id] == CVBS_FRAME_NTSC) {
		sensor_write(sd, 0xFF, ch_id);
		sensor_write(sd, 0x00, 0x00);
		sensor_write(sd, 0x06, 0x08);
		sensor_write(sd, 0x07, 0x63);
		sensor_write(sd, 0x2A, 0x81);
		sensor_write(sd, 0x3A, 0x20);
		sensor_write(sd, 0x3F, ch_id + 0x10);
		sensor_write_array(sd, sensor_cvbs_N_frame_mode_regs,
					 ARRAY_SIZE(sensor_cvbs_N_frame_mode_regs));
		sensor_write_array(sd, sensor_mipi_link_1,
					 ARRAY_SIZE(sensor_mipi_link_1));
		sensor_write(sd, 2 * ch_id + 0x78, 0x68);
		sensor_write(sd, 2 * ch_id + 0x79, 0x01);
		sensor_write_array(sd, sensor_mipi_link_2,
					 ARRAY_SIZE(sensor_mipi_link_2));
		sensor_print("tvin_info->input_fmt[ch_id] == CVBS_FRAME_NTSC\n");
	} else {
		sensor_write(sd, 0xFF, ch_id);
		sensor_write(sd, 0x00, 0x20);
		sensor_write(sd, 0x06, 0x08);
		sensor_write(sd, 0x07, 0x63);
		sensor_write(sd, 0x2A, 0x01);
		sensor_write(sd, 0x3A, 0x20);
		sensor_write(sd, 0x3F, ch_id + 0x10);
		sensor_write_array(sd, sensor_720p_regs,
					 ARRAY_SIZE(sensor_720p_regs));
		sensor_write_array(sd, sensor_mipi_link_1,
					 ARRAY_SIZE(sensor_mipi_link_1));
		sensor_write(sd, 2 * ch_id + 0x78, 0x80);
		sensor_write(sd, 2 * ch_id + 0x79, 0x02);
		sensor_write_array(sd, sensor_mipi_link_2,
					 ARRAY_SIZE(sensor_mipi_link_2));
		sensor_print("tvin_info->input_fmt[ch_id] == sensor_720p_regs\n");
	}
		...
		struct tvin_init_info *tvin_info)
{
	...
	if (sd->entity.stream_count != 0) {
		sensor_ch_reg_init(sd, &info->tvin.tvin_info);
		sensor_print("sensor_tvin_init sensor_ch_reg_init done\n");
	}
	info->tvin.flag = true;
				...
				struct tvin_ch_fmt *fmt)
{
	...
	/* need write 0xFF to switch page */
	sensor_write(sd, 0xFF, i);
	sensor_read(sd, 0x00, &rdval);

	if (GET_BIT(rdval, 4)) {
		fmt->lock = 0;  // no video
		sensor_print("ch%d is NO video input.\n", i);
	} else {
		fmt->lock = 1;  // on video
		sensor_print("ch%d is ON video input.\n", i);
	}
...
static const struct v4l2_subdev_pad_ops sensor_pad_ops = {
	...
	.set_fmt = rn6854m_sensor_set_fmt,
};
