...
//rpdzkj jeff    
	...
	ret = gtp_parse_dt_cfg(&ts->client->dev, &config[GTP_ADDR_LENGTH], (int *)&ts->gtp_cfg_len, sensor_id);
	if (ret < 0) {
...
//add end
	...
	printk("666sensor_id = %d\n",sensor_id);

/*
    memset(&config[GTP_ADDR_LENGTH], 0, GTP_CONFIG_MAX_LENGTH);
    memcpy(&config[GTP_ADDR_LENGTH], send_cfg_buf[sensor_id], ts->gtp_cfg_len);
*/    
	printk("777 sensor_id = %d\n",sensor_id);
